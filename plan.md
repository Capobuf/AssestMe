...
AssestMe supports two explicit database choices for a new installation: SQLite and MySQL / MariaDB. MySQL and MariaDB share PDO infrastructure but retain distinct product labels, version detection, executable validation, CI jobs, diagnostics, dump tools, and restore tools. For a server database, AssestMe establishes the PDO connection provisionally through MySQL and detects whether the connected product is MySQL or MariaDB. Only versions executed by the maintained CI matrix may be listed as compatible.
...
The hosting portability and server-backup correction started on 2026-08-01 from the clean `develop` HEAD `e881b45bd1aa77009ae8afa89ac30bd6fa9c2306`, exactly aligned with `origin/develop`. The bounded work corrects the MySQL/Percona dump tablespace option and sanitised diagnostics, makes failed or unavailable MySQL/MariaDB installer backups visible but non-blocking while retaining SQLite final-backup blocking behavior, broadens deterministic PHP CLI/WeasyPrint and SQL-client alias detection, checks private site-user permissions, adds panel-neutral scheduler/documentation guidance and a split-root shared-hosting entry template. It does not alter the Workspace, IndexedDB, offline persistence, models, schema, migrations, report generation, database-choice behavior, fixed name, PHP minimum, wizard back navigation, or general backup/restore behavior.

The hosting portability and server-backup correction completed its proportional verification on 2026-08-01. Docker Compose focused coverage passed `62` tests with `622` assertions across multi-driver backup, runtime inspector, installation wizard, release archive, and deployment configuration tests. Pint `--dirty` passed and scoped PHPStan over the nine changed application files reported zero errors. A real temporary release ZIP was built from the modified source and its central directory contains `assestme/deploy/shared-hosting/index.php.dist`. The complete `scripts/verify.sh` gate and a full Dusk run were not executed for this bounded installer/hosting slice.

The installer database-choice correction started on 2026-08-01 from the clean `develop` HEAD `39fa6302b3c3da2d5e2ffe1dc39f45da5e5fd2d6`, exactly aligned with `origin/develop`. The approved bounded change presents only SQLite and MySQL / MariaDB in the wizard, opens server connections provisionally through MySQL PDO, detects the actual server product after connection, persists the normalized real driver before classification, and adds direct back links between existing installer steps. The implementation is limited to the directly coupled requests, controller, probe, views, tests, and operator documentation; no installer reset, route, state machine, new DTO/service/enum, migration, backup-client discovery, PHP/WeasyPrint detection, IndexedDB, or Workspace behavior is changed.

The installer database-choice correction completed on 2026-08-01. The focused installer tests passed with `19` tests and `153` assertions. The real capability probe started provisionally as MySQL and passed against the Docker MySQL and MariaDB services, one test with `7` assertions for each product, returning the connected product driver. Pint `--dirty` and PHPStan over the five changed application files passed with zero errors. The complete Docker `scripts/verify.sh` gate completed; its current-fingerprint quality and browser receipts both match, covering the complete application suite, strict Canary, diagnostics, benchmark, storage audit, and isolated Dusk suite.
...
Pre-push proportional verification for the rolling develop prerelease passed on 2026-08-01. `git diff --check` and `bash -n scripts/build-cloudpanel-release.sh` passed. The host did not have Actionlint installed, so the official Actionlint `1.7.12` Linux amd64 release binary was used from a disposable directory and reported no workflow error. The Docker Compose focused `DeploymentConfigurationTest` passed `15` tests with `262` assertions, and the directly changed `CloudPanelReleaseScriptTest` passed `3` tests with `18` assertions. The required residual scan found no current archive name containing the CloudPanel suffix, no external ZIP sidecar reference, and no checksum-path variable outside `vendor` or Git metadata. The complete `scripts/verify.sh` gate was not run or claimed for this proportional CI-only slice; the hosted Quality run after push remains the required end-to-end packaging, installer, matrix, and publication evidence.

The hosted Quality workflow remediation started on 2026-08-01 from the clean `develop` HEAD `44fddbfd0f6663d70b059513121e29258f6453ae`, exactly aligned with `origin/develop`. Complete inspection of failed run `30669066028` confirmed seven failed jobs: a clean-checkout bootstrap that retained the example SQLite path and could not persist the generated application key, a CloudPanel browser installer missing its installed WeasyPrint binary path, both MariaDB services using the unavailable MySQL health command, both MySQL jobs requesting mutually conflicting MySQL and MariaDB client packages, and two PDF tests assuming URL annotations remain literal uncompressed byte strings. All four pinned Docker image manifests exist and pulled successfully. Proportional real-server reproduction then exposed one later workflow error hidden by the earlier service/APT stops: the matrix passed its MySQL or MariaDB test driver into three explicitly SQLite diagnostic regressions, producing three deterministic failures after the real capability, seed, and integrity tests had passed. No database version, dependency, application behavior, offline draft file, matrix coverage, renderer, or production contract is being changed. The bounded correction will make the existing bootstrap inputs concrete, select the product-authentic health check and client per existing matrix row, resolve the installed WeasyPrint executable, retain integrity tests in the server context while running the complete diagnostic regression class in its declared SQLite context, and assert PDF URL annotations through Poppler rather than PDF byte layout before proportional Docker verification and a new hosted workflow run.

Pre-push proportional verification passed on 2026-08-01. Checked Actionlint `1.7.12` reported no workflow error; `bash -n` and `git diff --check` passed; all four pinned Docker manifests resolved; both MariaDB images exposed `/usr/local/bin/healthcheck.sh` and no `mysqladmin`; and a disposable Compose checkout without `.env` completed dependency installation, persisted a new valid application key, migrated, seeded, created exactly one administrator, locked the installation, and passed diagnostics against its concrete SQLite file. Focused PDF plus deployment coverage passed `39` tests with `477` assertions, focused CloudPanel release/installer coverage passed `15` tests with `105` assertions, and the installable archive built with a valid SHA-256 sidecar. The complete extracted-release browser installer passed with `1` Dusk test and `13` assertions through SQLite configuration, permanent installer closure, real login, and authenticated diagnostics. Real MySQL `8.0.46` and MariaDB `11.8.6` seed and capability tests passed; server-context integrity regressions passed twice with `12` tests and `91` assertions each; the SQLite diagnostic class passed `7` tests with `78` assertions; and product-authentic MySQL and MariaDB dump/restore round trips each passed `1` test with `14` assertions. Targeted Pint passed for all three changed PHP test files. The authoritative complete gate was not repeated before hosted execution because the failed CI application suite had only the two reproduced PDF failures and every changed behavior was covered by the focused checks.

Hosted run `30693922341` on correction commit `3491897c0ac97d2d8854c70438ac62d50e343108` confirmed CloudPanel, MySQL `8.0.46`, and MariaDB `11.8.6` green and exposed two deterministic workflow-only errors that had been hidden by the original failures. The clean bootstrap now completes key persistence, dependency installation, migrations, seeding, administrator creation, installation locking, and database checks, but diagnostics rejects the blank example backup root; the workflow therefore supplies its already-created checkout `backups` directory explicitly. The recent-server functional command exits before running tests because eight historical filenames never existed; those paths are aligned to the current corresponding client/site, asset, risk, template, assessment, evidence, workspace, and import/export feature suites without removing coverage or a matrix row. A first exact MySQL `8.4.11` execution also proved that the separate `RiskMatrixFieldTest` cannot be added to this server list because its explicit rollback fixture at line 254 creates a SQLite-only `SELECT RAISE` trigger; it remains covered by the complete SQLite quality gate. The same hosted run's first quality attempt passed the complete isolated application gate but its browser server returned HTTP 500 responses and eventually refused connections; the uploaded artifact omitted the isolated server log. A same-SHA quality-only retry passed every browser assertion except the strict console check after Chromium `150.0.7871.124` emitted a `SEVERE` intervention for a blocked `beforeunload` dialog without user activation during automated navigation. The approved Docker/Selenium profile instead uses the project's locked Chromium `150.0.7871.114` and passed all `18` Dusk tests with `631` assertions in `176.45` seconds. The CI browser infrastructure is therefore aligned to that already-declared pinned Selenium image; no application, test, offline-persistence, dependency, or browser assertion is changed or filtered.

Second-round proportional verification used the exact recent matrix images. The aligned eight-file functional suite passed on MySQL `8.4.11` with `93` tests and `419` assertions in `46.07` seconds and on MariaDB `12.3.2` with the same `93` tests and `419` assertions in `29.87` seconds. Actionlint remained clean, the pinned Selenium manifest resolved, and a host-network container became healthy, reported Selenium `4.46.0` with Chromium `150.0.7871.114`, and stopped cleanly. The deployment/workflow regression class passed `14` tests with `245` assertions. The full Dusk suite was not repeated after these workflow-only edits because the exact pinned browser image had already passed it completely once; the new hosted run remains the required end-to-end evidence.

Hosted run `30694691774` on `f76e4697d4918fc67fc8a55fc1a127dceb97cd39` made CloudPanel, clean checkout, and all four database jobs green, including both recent-server functional suites and their real dump/restore round trips. The quality job's complete application gate and pinned Selenium bootstrap also passed, but the browser suite again produced intermittent HTTP 500 responses after several passing classes; eight later tests then failed from missing UI or the same 500. Because the isolated server log was still outside the uploaded checkout paths, no database-lock, PHP, or application cause is claimed. The next workflow-only diagnostic keeps the marked Dusk root under `$RUNNER_TEMP` and uploads its server log on failure without changing, filtering, retrying, or skipping a browser assertion.

Hosted diagnostic run `30694968394` on `034ea80217e2b40f524146924fe5d22594f69a69` again made clean checkout, CloudPanel, and all four database jobs green. Its uploaded isolated server log identifies the first browser failure directly: the PHP `8.3.33` CLI development-server process terminated with `Segmentation fault` while serving concurrent browser requests; the eighteen Dusk `ERR_CONNECTION_REFUSED` failures are consequences after that process exited, not application assertions or offline-persistence failures. The project-owned development runtime is already locked to PHP `8.3.32`, and its complete isolated Dusk execution with the same pinned Selenium image passed `18` tests and `631` assertions; the focused rerun of the first affected class also passed `1` test and `135` assertions.

Hosted run `30695259546` proved that asking `setup-php` for `8.3.32` does not select that patch on this runner: its own setup log still reports `Updated to PHP 8.3.33`, and the isolated PHP server again terminates with `Segmentation fault`; all six non-browser jobs and the complete non-Dusk application gate remained green. The ineffective host-runtime constraint is removed. The browser gate now runs through the already-authoritative Docker Compose development profile so both PHP `8.3.32` and Selenium/Chromium are the project-locked, previously verified artifacts. The complete Dusk suite and every assertion remain enabled, its isolated server log remains uploadable from a marked disposable root, and no application code, dependency, offline-persistence file, browser test, assertion, or matrix entry is changed.
