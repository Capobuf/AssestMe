...
| D-057 (v3) | APPROVED | Filament 5 native clusters and parent items define navigation; Asset is a primary menu cluster containing assets and asset types while sites remain below companies; Tag is removed from v1; Finding assets are optional for every scope, with a conditional non-required selector that can create a minimal Asset bound to the Assessment company and create its enabled Asset type inline; and implementation verification is proportional under the testing contract in ADR 0008 |
