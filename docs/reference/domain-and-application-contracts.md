...
- Monetary estimate types are single amount (`Stima`) and range. The former `Esatta` type is
  consolidated into `Stima`; existing rows and the legacy `exact` interchange value are normalized
