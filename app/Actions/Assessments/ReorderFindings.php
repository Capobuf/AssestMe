...
final class ReorderFindings
{
     ...
     */
    public function __invoke(Assessment $assessment, array $orderedFindingIds): int
    {
        $expectedVersion = (int) $assessment->lock_version;

        return Cache::lock("assessment:{$assessment->getKey()}:save", 10)->block(
            5,
            fn (): int => DB::transaction(function () use ($assessment, $orderedFindingIds, $expectedVersion): int {
