...
it('installs the extracted release with production-safe configuration checks', function (): void {
    ...
    expect($script)
        ...
        ->toContain('RELEASE_SERVER_LAUNCH_MODE')
        ->toContain('Composer\\InstalledVersions::getInstallPath("laravel/framework")')
        ->toContain('RELEASE_DIRECT_SERVER_WORKING_DIRECTORY=$project_path/public')
        ->toContain('cd "$project_path/public"')
        ->toContain('$php_binary" -S "127.0.0.1:$port" "$laravel_server_router"')
